...
class OrderService {
  ...
  /**
   * Helper to check product availability for a given period.
   ...
   */
  async checkProductAvailability(tx, productId, requestedQty, rentalStart, rentalEnd) {
    ...
    const availableQty = totalCapacity - committedQty;

    if (availableQty < requestedQty) {
      throw {
        statusCode: 400,
        message: `Insufficient stock for product "${product.name}" for the chosen period. Available: ${availableQty}, Requested: ${requestedQty}`
      };
    }
  }
  ...
  async createQuotation(orderData, customerId) {
    ...
    for (const item of lines) {
      ...
      if (ratePrice <= 0) {
        throw { statusCode: 400, message: 'Price must be greater than zero' };
      }

      // Rule 2: Check availability for the requested period before booking
      await this.checkProductAvailability(prisma, product.id, quantity, startDate, endDate);
  ...
  async checkout(orderData, customerId) {
    ...
    if (!['CASH', 'CARD', 'UPI', 'WALLET'].includes(method)) {
      throw { statusCode: 400, message: `Invalid payment method: ${paymentMethod}` };
    }
    ...
    return prisma.$transaction(async (tx) => {
      // Rule 2: Check availability inside the transaction before booking
      for (const item of lines) {
        await this.checkProductAvailability(tx, parseInt(item.productId, 10), parseInt(item.qty || 1, 10), startDate, endDate);
      }
      ...
      if (method === 'WALLET') {
        ...
        const totalToPay = orderGrandTotal + calculatedSecurityDepositTotal;
        if (customer.walletBalance < totalToPay) {
          throw { statusCode: 400, message: `Insufficient wallet balance (Required: ₹${totalToPay.toFixed(2)}, Available: ₹${customer.walletBalance.toFixed(2)})` };
        }
        
        // Deduct payment from wallet (never allow negative wallet)
        await tx.user.update({
          where: { id: customer.id },
          data: { walletBalance: { decrement: totalToPay } },
        });
      }
  ...
  async confirmOrder(id) {
    ...
    return prisma.$transaction(async (tx) => {
      // Rule 2: Check availability before confirming
      for (const line of order.lines) {
        if (line.lineType === 'RENTAL' && line.productId) {
          await this.checkProductAvailability(tx, line.productId, line.qty, order.rentalStart, order.rentalEnd);
        }
      }
    ...
    return prisma.$transaction(async (tx) => {
      for (const line of order.lines) {
        if (line.lineType === 'RENTAL' && line.productId) {
          ...
          if (!product) {
            throw { statusCode: 404, message: `Product ${line.productId} not found` };
          }
          
          // Rule 5: Stock can never go below 0 — guard it in the service
          const newQty = product.qtyOnHand - line.qty;
          if (newQty < 0) {
            throw { statusCode: 400, message: `Cannot pickup: insufficient warehouse stock for product "${product.name}". Available on hand: ${product.qtyOnHand}, Requested: ${line.qty}` };
          }
          
          // Reduce physical quantity on hand
  ...
  async cancelOrder(id) {
    ...
    return prisma.$transaction(async (tx) => {
      // Rule 4: On Cancel: restore the quantity (only if it was physically picked up)
  ...
  async returnOrder(id, actualReturnTime) {
    ...
    return prisma.$transaction(async (tx) => {
      ...
      // Rule 1: Deposit settlement on return - deposit floors at 0.
      // If late fee is larger than the deposit, deduct the full deposit (refund = 0)
      // and record the payment applied to the late fee from the deposit as only the deposit amount.
      // The remaining unpaid late fee becomes outstanding.
      let refundAmount = 0;
      let depositSettlementStatus = 'REFUNDED';
      let lateFeePaidFromDeposit = 0;

      if (order.depositAmount > 0) {
        // Find the original deposit payment to see how it was paid
        const depositPayment = order.payments.find(p => p.type === 'DEPOSIT' && p.amount > 0);
        const isWalletPayment = depositPayment && depositPayment.method === 'WALLET';
        const refundMethod = isWalletPayment ? 'WALLET' : 'UPI';

        if (lateFeeTotalAmount > 0) {
          refundAmount = Math.max(order.depositAmount - lateFeeTotalAmount, 0);
          lateFeePaidFromDeposit = Math.min(lateFeeTotalAmount, order.depositAmount);
        ...
        // If paid via wallet and refund exists, increment wallet balance (never allow negative wallet)
        ...
        // Settle only up to the deposit amount so the leftover late fee remains outstanding
        if (lateFeePaidFromDeposit > 0) {
          await tx.payment.create({
            data: {
              orderId: order.id,
              amount: lateFeePaidFromDeposit,
              method: refundMethod,
              type: 'LATE_FEE',
              date: actualReturnDate,
            },
      ...
      // Step 4: Increment product booking statistics and restore quantity on hand
      const rentalLines = order.lines.filter((line) => line.lineType === 'RENTAL' && line.productId);
      ...
      const leftoverLateFee = Math.max(lateFeeTotalAmount - order.depositAmount, 0);
      await tx.notification.create({
        data: {
          userId: order.customerId,
          orderId: order.id,
          message: isLateReturn
            ? `Your rental ${order.reference} was returned ${totalOverdueHours} hours late. Late fee of ${lateFeeTotalAmount.toFixed(2)} charged. Deposit deducted: ${lateFeePaidFromDeposit.toFixed(2)}. Leftover outstanding: ${leftoverLateFee.toFixed(2)}.`
