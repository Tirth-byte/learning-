...
import { Card, Descriptions, Typography, Tag, Button, Table, Row, Col, Space, Spin, message, Modal, Input, Radio, Alert } from 'antd';
...
const OrderDetail = () => {
  ...
  const totalPaid = order
    ...
    : 0;
  const amountDue = order ? Math.max(order.total - totalPaid, 0) : 0;
  ...
  const fetchOrderDetails = async () => {
    try {
      ...
      setOrder(ord);
      if (ord) {
        const tp = (ord.payments || [])
          .filter(p => p.type === 'RENTAL' || p.type === 'LATE_FEE')
          .reduce((sum, p) => sum + p.amount, 0);
        const ad = Math.max(ord.total - tp, 0);
        setPayAmount(ad);
      }
    } catch (error) {
  ...
  // Silent polling background update every 4 seconds
  const pollOrderDetails = async () => {
    if (document.hidden || payModalOpen) return; // Pause when hidden or input modal is active
    try {
      setIsUpdating(true);
      const orderRes = await api.get(`/orders/${id}`);
      const ord = orderRes.data.data;
  ...
  const handleRecordPayment = async () => {
    ...
    if (payAmount > amountDue + 0.01) {
      message.error(`Payment cannot exceed the outstanding amount due (₹${amountDue.toFixed(2)})`);
      return;
    }
            ...
            <Row justify="end" style={{ marginTop: 16 }}>
              <Col span={8}>
                <Descriptions column={1} size="small" colon={false} labelStyle={{ color: '#6B7280' }} contentStyle={{ textAlign: 'right', display: 'block' }}>
                  ...
                  <Descriptions.Item label="Total Amount" contentStyle={{ fontSize: 14, fontWeight: 600, color: '#374151' }}>
                    ₹{order.total.toFixed(2)}
                  </Descriptions.Item>
                  <Descriptions.Item label="Amount Paid" contentStyle={{ fontSize: 14, fontWeight: 600, color: '#16A34A' }}>
                    ₹{totalPaid.toFixed(2)}
                  </Descriptions.Item>
                  <Descriptions.Item label="Outstanding Amount Due" contentStyle={{ fontSize: 16, fontWeight: 700, color: amountDue > 0 ? '#DC2626' : '#16A34A' }}>
                    ₹{amountDue.toFixed(2)}
                  </Descriptions.Item>
                </Descriptions>
              </Col>
