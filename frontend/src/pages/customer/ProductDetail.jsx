...
const ProductDetail = () => {
  ...
  const fetchProductDetail = async (currentDates = dates) => {
    try {
      setLoading(true);
      let url = `/products/${id}`;
      if (currentDates && currentDates[0] && currentDates[1]) {
        url += `?rentalStart=${currentDates[0].toISOString()}&rentalEnd=${currentDates[1].toISOString()}`;
      }
  ...
  useEffect(() => {
    if (dates && dates[0] && dates[1]) {
      const url = `/products/${id}?rentalStart=${dates[0].toISOString()}&rentalEnd=${dates[1].toISOString()}`;
      api.get(url)
        .then(res => {
          if (res.data?.data) {
            setProduct(prev => {
              if (!prev) return null;
              return { ...prev, availableQty: res.data.data.availableQty };
            });
          }
        })
        .catch(err => {
    ...
    } else if (product && product.availableQty !== undefined) {
      setProduct(prev => {
        if (!prev) return null;
        const copy = { ...prev };
        delete copy.availableQty;
        return copy;
      });
    }
  }, [dates, id]);
  ...
  const handleAddToCart = () => {
    ...
    const displayAvailable = product ? (product.availableQty !== undefined ? product.availableQty : product.qtyOnHand) : 0;
    if (quantity > displayAvailable) {
      message.error(`Quantity cannot exceed available stock (${displayAvailable} units) for this period.`);
      return;
    }
  ...
  // Calculate quantity error message dynamically without auto-correcting input
  let qtyError = '';
  if (quantity !== null && quantity !== undefined) {
    if (quantity <= 0) {
      qtyError = 'Quantity must be greater than zero.';
    } else if (quantity > displayAvailable) {
      qtyError = `Quantity cannot exceed available stock (${displayAvailable} units) for the chosen period.`;
    }
  }

  const isAvailableForPeriod = quantity > 0 && !qtyError && displayAvailable >= quantity;
  const inStock = displayAvailable > 0;
      ...
      <div className="sf-detail">
        ...
        <div className="sf-booking">
          <div className="u-row u-wrap">
            ...
            <StatusPill tone={isAvailableForPeriod ? 'success' : 'warning'}>
              {dates ? `${displayAvailable} available for selected dates` : `${displayAvailable} available`}
            </StatusPill>
          </div>
          ...
          <div className="sf-spec-grid">
            ...
            <div className="sf-spec">
              <span className="sf-spec-label">{dates ? 'Available' : 'In stock'}</span>
              <span className="sf-spec-value">{displayAvailable} units</span>
            </div>
          </div>
          ...
          <div className="sf-field">
            ...
            />
            {qtyError && (
              <div style={{ color: 'var(--danger)', fontSize: 'var(--t-xs)', marginTop: 4, fontWeight: 500 }}>
                {qtyError}
              </div>
            )}
          </div>
          ...
          >
            {isAvailableForPeriod ? 'Add to cart' : (displayAvailable <= 0 ? 'Out of stock' : 'Insufficient availability')}
          </Button>
