            ...
            <Surface title="Payment">
              ...
              <Form layout="vertical" requiredMark={false} onFinish={processCompleteCheckout}>
                <Form.Item label="Payment method">
                  <Radio.Group value={selectedPaymentMethod} onChange={e => setSelectedPaymentMethod(e.target.value)}>
                    <Radio value="card">Card</Radio>
                    <Radio value="upi">UPI</Radio>
                    <Radio value="wallet">Wallet (Balance: {formatCurrency(walletBalance)})</Radio>
                  </Radio.Group>
                </Form.Item>

                {selectedPaymentMethod === 'wallet' && walletBalance < dueToday && (
                  <div style={{ marginBottom: 16, padding: '12px 16px', background: '#FEF2F2', border: '1px solid #FCA5A5', borderRadius: 6, color: '#991B1B', fontSize: 13 }}>
                    Insufficient wallet balance. You have {formatCurrency(walletBalance)}, but this order requires {formatCurrency(dueToday)}. Please select another payment method.
                  </div>
                )}
                ...
                {selectedPaymentMethod === 'upi' && (
                  <div style={{ marginBottom: 16, padding: '12px 16px', background: '#F0FDF4', border: '1px solid #86EFAC', borderRadius: 6, color: '#166534', fontSize: 13 }}>
                    UPI Payment: Proceed to pay {formatCurrency(dueToday)} using your default UPI application.
                  </div>
                )}
                ...
                <div className="u-row" style={{ marginTop: 'var(--s-2)' }}>
                  ...
                  <Button
                    ...
                    htmlType="submit"
                    loading={loading}
                    disabled={selectedPaymentMethod === 'wallet' && walletBalance < dueToday}
